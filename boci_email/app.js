...
function splitFreeText(source) {
  const match = source.match(/\*\*Free text:\*\*\s*([\s\S]*)/i);
  ...
  const firstChineseBullet = chineseBlock.search(/\(i\)/i);
...
function buildEmailHtml(report) {
  ...
  const englishTitle = titleParts.remainder
  ...
  const englishBulletHtml = report.englishBullets
    .map((bullet) => `<p style="margin-top:0px;margin-Bottom:120px"><span style="${EMAIL_STYLES.englishBody}"><b>(${escapeHtml(bullet.marker)}) ${emphasizeEnglish(bullet.body)} </b></span></p>`)
    .join("\n");
  const chineseBulletHtml = report.chineseBullets
    .map((bullet) => `<p style="margin-top:0px;margin-Bottom:80px"><span style="${EMAIL_STYLES.englishBody}"><b>(${escapeHtml(bullet.marker)}) </b></span>${renderMixedFontRuns(bullet.body, EMAIL_STYLES.chineseBody, EMAIL_STYLES.englishBody)}</p>`)
    .join("\n");
  ...
  return `
    ...
    ${englishBulletHtml}
    <p style="margin-top:0px;margin-Bottom:120px"><span style="${EMAIL_STYLES.englishBody}"><b>&nbsp;</b></span></p>
    <p class="zh-title" style="margin-top:80pt;margin-Bottom:20pt">${renderMixedFontRuns(chineseTitle, EMAIL_STYLES.chineseTitle, EMAIL_STYLES.chineseTitleLatin)}</p>
    <p class="analysts" style="margin-top:0px;margin-Bottom:20px">${chineseAnalysts}</p>
    ${chineseBulletHtml}
